...
def add_tartan_args(parser):
	...
	parser.add_argument("--primary_task_id", type=str, default='citation_intent')
...
class TartanModel(AutoModelWithLMHead):
	...
	def setup_datasets(self, dataset_files, model_name, max_seq_len, label_vocab=None, lazy=False):
		...
		indexers = {'tokens': PretrainedTransformerIndexer(model_name)}
		...
		pretrain_vocab = tokenizer.tokenizer.get_vocab()
		#tokenizer._tokenizer.encoder
		for idx_, fname in dataset_files.items():
			...
			for instance in all_samples:
				tokens_ = instance.fields['tokens']
				# print("tokens")
				# print(tokens_)
				...
				for t in tokens_:
					...
					# print(type(pretrain_vocab))
					if t in pretrain_vocab:
						tokens.append(pretrain_vocab[t])
					else:
						# print(t)
						tokens.append(tokenizer.tokenizer.unk_token_id) #UNK

				if len(tokens) > max_seq_len:
					tokens_[:max_seq_len-1] += [tokenizer.tokenizer.sep_token]
					tokens[:max_seq_len-1] += [tokenizer.tokenizer.sep_token_id]

				for t in range(max_seq_len - len(tokens)):#PAD
					tokens_.append(tokenizer.tokenizer.pad_token)
					tokens.append(tokenizer.tokenizer.pad_token_id)

				assert(
					len(tokens)==max_seq_len and len(instance)==max_seq_len,
					'Length が異なる'
					)

				sentence = torch.tensor(tokens)#.to(self.base_model.device)
				# tokens_ = [Token(t) for t in tokens_]
				sentence = TensorField(sentence)
				sentence = sentence.tensor #.as_tensor(max_seq_len)
			...
			datasets[idx_] = {
								...
								'pad_idx': tokenizer.tokenizer.pad_token_id,
								'vocab': vocab
							}
		return datasets
